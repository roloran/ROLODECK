...
void persistence_setup(void)
{
  ...
  if(FFat.begin(true)) // Format file system if necessary
  {
    hasFFat = true;
  }
  else
...
void persistence_add_basecfg(String s)
{
  if (hasFFat)
  {
    File f = FFat.open(FILENAME_DEVICE_SETTINGS, FILE_APPEND);
    if (!f) return;
    f.print(s + "\n");
    f.close();
  }
...
void persistence_dump_basecfg(void)
{
  if (hasFFat)
  {
    File f = FFat.open(FILENAME_DEVICE_SETTINGS, FILE_READ);
...
void persistence_reset_replay_serial(void)
{
  if (hasFFat)
  {
    FFat.remove(FILENAME_SERIAL_REPLAY);
  }
  return;
}
...
void persistence_delete_basecfg(void)
{
  if (hasFFat)
  {
    FFat.remove(FILENAME_DEVICE_SETTINGS);
  }
  return;
}
